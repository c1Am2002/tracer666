...
1. 在视图函数中
    ...
    page_object = Pagination(
        current_page=request.GET.get('page'),获取当前url上的页码， 由用户输入， 或者点击跳转
        all_count=queryset.count(),
        base_url=request.path_info, 当前rul的前缀
...
class Pagination(object):
    def __init__(self, current_page, all_count, base_url, query_params, per_page=30, pager_page_count=11):
        ...
        :param base_url: 基础URL ,想生成的url的前缀
        :param query_params: QueryDict对象，
        内部含所有当前URL的原条件 request.get, 防止用户额外给url添加属性， 如果不设置这个， 而以我们的格式生成url的话会删除用户的属性，
